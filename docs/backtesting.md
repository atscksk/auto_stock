...
TQQQ의 장기 추세를 QQQ 기준으로 판단하려면 `--trendFile`과 `--trendSymbol`을 함께 사용한다.

```bash
npm run infinite:backtest -- --file data/TQQQ-2026-H1.csv --trendFile data/QQQ-2026-H1.csv --trendSymbol QQQ --symbol TQQQ --cash 10000 --strategyCapital 10000
```

SOXL은 기본 기준 ETF로 SOXX를 사용한다. SMH를 기준으로 테스트하려면 `--trendSymbol SMH`와 SMH 캔들 파일을 지정하면 된다.

```bash
npm run infinite:backtest -- --file data/SOXL-2026-H1.csv --trendFile data/SOXX-2026-H1.csv --trendSymbol SOXX --symbol SOXL --cash 10000 --strategyCapital 10000
```

기준 ETF 캔들이 없으면 대상 종목 캔들로 MA200을 대체 판단한다. 예를 들어 TQQQ 백테스트에서 QQQ 캔들을 넣지 않으면 TQQQ MA200으로 fallback하며, 리포트 경고에 fallback 여부가 표시된다. 대상 종목 캔들이 200개 미만이면 신규 사이클 매수가 차단될 수 있다.

종목별 기본 리스크 프리셋도 적용된다. TQQQ는 기본 기준을 사용하고, SOXL은 변동성이 더 크므로 현금 보존 비율과 손실 기준을 더 보수적으로 둔다.

| 종목 | 현금 보존 | 당일 급락 | 손실 일시중단 | 수동중단 | 손절중단 | 연속 하락 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| TQQQ | 20% | -8% | -30% | -45% | -60% | 5일 중 4일 |
| SOXL | 25% | -12% | -35% | -50% | -65% | 6일 중 5일 |
...
--trendFile            기준 ETF 캔들 CSV 경로. 예: QQQ 캔들로 TQQQ MA200 필터 판단
--trendSymbol          기준 ETF 심볼. 예: QQQ
--enableBigBuy         큰수매수 조건 판단 활성화
--disableReverseMode   리버스모드 진입 조건 판단 비활성화
...
주의: 현재 무한매수 구현은 V4 전체 기능이 완성된 상태가 아니다. 일반 LOC 매수, 별표 매도, 15% 보조 매도, 매도거부, 큰수매수, 스킵, 리버스모드, EXIT_WAIT, 상태별 주문 허용 정책은 일부 반영되었지만 실제 체결 동기화와 운영 안정성은 보강이 필요하다.
